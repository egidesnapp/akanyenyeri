<?php
/**
 * Theme-compatible content template
 * Expects an associative array `$post` with keys: id, title, slug, excerpt, content, featured_image, created_at, author_name, category_name, category_slug
 */
...
<?php $aos_attrs = '';
if (isset($aos_delay)) {
    $aos_attrs = ' data-aos="fade-up" data-aos-delay="' . intval($aos_delay) . '"';
}
?>
<article id="post-<?php echo htmlspecialchars($post['id']); ?>" class="post type-post status-publish format-standard<?php echo $post['featured_image'] ? ' has-post-thumbnail' : ''; ?> hentry"<?php echo $aos_attrs; ?>>
